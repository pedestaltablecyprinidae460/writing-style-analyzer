...
class TestProfileStructure:
    ...
    def test_profile_has_required_top_level_fields(
        ...
        for field in required_fields:
            assert field in sample_profile_default, f"Profile missing required field: {field}"
    ...
    def test_profile_has_required_metric_fields(
        ...
        for field in required_metric_fields:
            assert field in metrics, f"Profile.metrics missing required field: {field}"
...
class TestProfileDataValidity:
    ...
    def test_lexical_diversity_in_valid_range(self, sample_profile_default):
        ...
        assert 0 < lexical_div <= 1, f"lexical_diversity must be in (0, 1], got {lexical_div}"
    ...
    def test_passive_ratio_in_valid_range(self, sample_profile_default):
        ...
        assert 0 <= passive_ratio <= 1, f"passive_ratio must be in [0, 1], got {passive_ratio}"
    ...
    def test_transition_categories_have_patterns(self, sample_profile_default):
        ...
        for category, patterns in transitions.items():
            assert isinstance(patterns, list), f"Transition category '{category}' must be a list"
            assert (
                len(patterns) > 0
    ...
    def test_transition_patterns_are_strings(self, sample_profile_default):
        ...
        for category, patterns in transitions.items():
            for pattern in patterns:
                ...
                assert len(pattern.strip()) > 0, f"Pattern in '{category}' cannot be empty"
...
class TestProfileConsistency:
    ...
    def test_avg_sentence_length_matches_totals(self, sample_profile_default):
        ...
        assert diff < 0.5, (
            f"avg_sentence_length inconsistent: " f"expected {expected_avg:.2f}, got {actual_avg}"
        )
...
class TestProfileComparison:
    ...
    def test_excellence_has_higher_transition_density(
        ...
        excellence_density = sample_profile_excellence["metrics"]["transitions"][
            "transition_density"
        ]
    ...
    def test_excellence_has_lower_passive_ratio(
        ...
        excellence_passive = sample_profile_excellence["metrics"]["voice_and_style"][
            "passive_ratio"
        ]
